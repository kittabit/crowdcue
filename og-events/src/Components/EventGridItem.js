...
function EventGridItem(props) {
    ...
    return (
        <>  
            ...
            <div className="col-span-3 bg-slate-100 rounded-lg h-52 md:h-80 no-underline">
                <Link to={`/events/details/${ props.item.slug }/`} className="no-underline">
                    <img src={ props.item.image_url } alt={ props.item.name } className="rounded-t-lg max-h-44" loading="lazy" />
